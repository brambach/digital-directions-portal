...
export default async function AdminTicketDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#F4F5F9] p-8 space-y-6 no-scrollbar relative font-geist">
      <AnimateOnScroll />
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6 animate-enter delay-200">
          ...
          {/* Ticket Body Card */}
          <Card className="rounded-2xl border-gray-100 shadow-sm overflow-visible">
            <div className="p-8">
              {/* Title Row */}
              <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900 tracking-tight leading-tight mb-2">
                  {ticket.title}
                  <span className="text-base text-gray-400 font-medium ml-2">#{ticket.id.slice(0, 8)}</span>
                </h1>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {creator?.email && <span className="text-sm text-gray-400">&lt;{creator.email}&gt;</span>}
                ...
                <div key={comment.id} className="flex gap-4 animate-enter" style={{ animationDelay: `${idx * 0.05}s` }}>
                  <div className="flex-shrink-0 mt-1">
                    {author?.avatar ? (
                      ...
                      <div className="w-9 h-9 rounded-full bg-purple-50 flex items-center justify-center border border-purple-100 text-purple-700">
                    ...
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        {author?.role === "admin" && !comment.isInternal && (
                          <span className="text-[9px] bg-purple-50 text-purple-700 px-1.5 py-0.5 rounded-full font-bold border border-purple-100 uppercase tracking-wide">DD Team</span>
                        )}
                      </div>
                      <span className="text-xs text-gray-400 font-medium">{formatDistanceToNow(new Date(comment.createdAt), { addSuffix: true })}</span>
                    </div>
                    <Card className={cn("p-5 rounded-xl border-gray-100 shadow-sm relative", comment.isInternal ? "bg-amber-50/30 border-amber-100" : "bg-white")}>
            ...
            {ticket.resolution && (
              <div className="flex gap-4 animate-enter">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-gray-900">Resolved</span>
        ...
        {/* Right Sidebar */}
        <div className="col-span-12 lg:col-span-4 space-y-5 animate-enter delay-300">
          ...
          {/* Requester Info */}
          <div className="space-y-1">
            <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-2 mb-2">Requester</h3>
            <Card className="p-5 rounded-xl border-gray-100 shadow-sm bg-white">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-[12px] text-gray-400 truncate">{creator?.email}</p>
                </div>
              </div>
              {ticket.clientName && (
                <div className="flex items-center gap-2 text-[12px] text-gray-500 mb-3 px-1">
              ...
              <div className="pt-3 border-t border-gray-50">
                <Link href={`/dashboard/admin/clients/${ticket.clientId}`}>
                  <Button variant="outline" className="w-full rounded-xl border-gray-200 text-xs font-bold uppercase tracking-wide">View Client</Button>
                </Link>
              </div>
            </Card>
          </div>
          ...
          <div className="space-y-1">
            ...
            <Card className="p-5 rounded-xl border-gray-100 shadow-sm bg-white">
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Priority</label>
                  <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                    <TicketPriorityBadge priority={ticket.priority} size="sm" />
                  </div>
                </div>
                <div>
                  <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Assigned To</label>
                ...
                {ticket.projectName && (
                  <div>
                    <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Project</label>
                    <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
