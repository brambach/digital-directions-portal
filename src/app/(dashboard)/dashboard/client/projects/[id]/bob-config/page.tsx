...
import { ClientBobConfigContent } from "@/components/client-bob-config-content";

export const dynamic = "force-dynamic";

export default async function ClientBobConfigPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const status = deriveStageStatus("bob_config", project.currentStage);
  ...
  return (
    <div className="min-h-full bg-[#F4F5F9] px-7 py-6 space-y-6">
      ...
      <StageCard
        ...
        description="Work through the HiBob configuration checklist to prepare your HR data for integration."
        isAdmin={false}
        projectId={id}
        backHref={`/dashboard/client/projects/${id}`}
      >
        <ClientBobConfigContent projectId={id} />
        {status !== "locked" && (
